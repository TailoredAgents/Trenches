...
export function chooseSize(ctx: SizingContext & { rugProb?: number; pFill?: number; expSlipBps?: number }): SizeDecision {
  ...
  const candidates = arms.map((a: any) => ({ arm: `${a.type}:${a.value}`, notional: Math.max(0, Math.min(ctx.walletFree, equity * a.value)) }));
  ...
  try {
    ...
    const exps = scores.map((s) => Math.exp((s - max) / Math.max(1e-6, max || 1)));
    const sum = exps.reduce((a, b) => a + b, 0) || 1;
    const probs = exps.map((e) => e / sum);
    const armIndex = candidates.findIndex((c) => c.arm === best.arm);
    const ctxHash = Math.abs(JSON.stringify({ ts, mint: ctx.candidate.mint }).split('').reduce((a, c) => ((a << 5) - a + c.charCodeAt(0)) | 0, 0));
