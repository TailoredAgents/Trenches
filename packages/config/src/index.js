...
const DEFAULT_CONFIG_PATH = path_1.default.resolve(process.cwd(), 'config', 'default.yaml');
...
const baseConfig = schema_1.configSchema.parse({
    mode: 'FULL',
    ...
    topics: {
        ...
        baseline: { halfLifeSec: 604800, flushIntervalSec: 300 },
        test: { enabled: false }
    },
    ladders: {
...
function applyEnv(config) {
    ...
    for (const [pathKey, envKey, caster] of envMap) {
        ...
        if (envVal !== undefined && envVal !== '') {
            setPath(mutated, pathKey, caster(envVal));
        }
    }
