...
const TRAINING_VIEW_DDLS = [
  'DROP VIEW IF EXISTS fill_training_view;',
  `CREATE VIEW fill_training_view AS
SELECT
...
WHERE (CASE WHEN se.ts > 20000000000 THEN se.ts/1000 ELSE se.ts END) >= strftime('%s','now') - 90*24*3600;`,
  'DROP VIEW IF EXISTS alpha_training_view;',
  `CREATE VIEW alpha_training_view AS
WITH all_exec AS (
...
LEFT JOIN win60 w60 ON w60.mint = e.mint;`,
  'DROP VIEW IF EXISTS rug_training_view;',
  `CREATE VIEW rug_training_view AS
WITH good AS (
...
WHERE y_bad IS NOT NULL;`,
  'DROP VIEW IF EXISTS survival_training_view;',
  `CREATE VIEW survival_training_view AS
WITH all_exec AS (
  SELECT mint, (CASE WHEN ts > 20000000000 THEN ts/1000 ELSE ts END) AS ts, exec_price
  FROM exec_outcomes
  WHERE (CASE WHEN ts > 20000000000 THEN ts/1000 ELSE ts END) >= strftime('%s','now') - 90*24*3600 AND mint IS NOT NULL AND filled = 1
...
// Training data materialized views
MIGRATIONS.push({
  id: '0018_training_views',
  statements: [
    (db) => ensureColumn(db, 'exec_outcomes', 'mint', 'TEXT'),
    (db) => ensureColumn(db, 'exec_outcomes', 'order_id', 'TEXT'),
    (db) => ensureColumn(db, 'exec_outcomes', 'side', "TEXT DEFAULT 'buy'"),
    ...TRAINING_VIEW_DDLS
  ]
});

function ensureTrainingViews(db: DatabaseConstructor.Database): void {
  ensureTable(
    db,
  ...
  ensureColumn(db, 'exec_outcomes', 'mint', 'TEXT');
  ensureColumn(db, 'exec_outcomes', 'order_id', 'TEXT');
  ensureColumn(db, 'exec_outcomes', 'side', "TEXT DEFAULT 'buy'");
  for (const ddl of TRAINING_VIEW_DDLS) {
    try {
      db.exec(ddl);
    } catch (err) {
      logger.warn({ err, ddl }, 'training view bootstrap warning');
    }
  }
}
